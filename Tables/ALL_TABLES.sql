...
DEFAULT CHARACTER SET = latin1;
 ...
 CREATE VIEW VW_APURACAO_FATURAMENTO_DETALHADO AS 
   SELECT PR.COD_PRODUTO, PR.NME_PRODUTO, SUM(ITOP.QTD_ITEM)QTD_ITEM , AVG(ITOP.VLR_ITEM) VLR_PRECO_MEDIO, OP.DTA_OPERACAO, OP.TPO_OPERACAO, PR.QTD_ESTOQUE_MIN, PR.QTD_ESTOQUE_MAX
		FROM OPERACAO OP
 ...
 ; 
	 ...
	 create view VW_APURACAO_FATURAMENTO_SUMARIZADO    as 
   SELECT 	A.COD_PRODUTO, 	
			(B.QTD_ITEM * a.VLR_PRECO_MEDIO) VLR_CUSTO_VENDA,
			(B.QTD_ITEM * B.VLR_PRECO_MEDIO) VLR_FATURADO, 
			(B.QTD_ITEM * B.VLR_PRECO_MEDIO) - (B.QTD_ITEM * A.VLR_PRECO_MEDIO) VLR_LIQUIDO, 
            A.QTD_ITEM QTD_ITEM_ESTOQUE_ENTRADA, 
            B.QTD_ITEM QTD_ITEM_ESTOQUE_SAIDA, 
            ROUND(b.QTD_ITEM / A.QTD_ITEM * 100, 2)  VLR_PCT_ESTOQUE_UTILIZADO, 
            B.DTA_OPERACAO
		FROM 	(SELECT QTD_ITEM, VLR_PRECO_MEDIO , COD_PRODUTO, cast(DTA_OPERACAO as date) DTA_OPERACAO
					FROM VW_APURACAO_FATURAMENTO_DETALHADO WHERE TPO_OPERACAO = 'C') A 
		INNER JOIN 
				(SELECT QTD_ITEM, VLR_PRECO_MEDIO , COD_PRODUTO, cast(DTA_OPERACAO as date) DTA_OPERACAO, QTD_ESTOQUE_MIN, QTD_ESTOQUE_MAX
					FROM VW_APURACAO_FATURAMENTO_DETALHADO WHERE TPO_OPERACAO = 'V') B
